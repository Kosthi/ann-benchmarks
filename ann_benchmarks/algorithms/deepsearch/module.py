import deepsearch as deepsearch
import numpy as np
...
def metric_mapping(_metric: str):
    ...
    if _metric_type is None:
        raise Exception(f"[deepsearch] Not support metric type: {_metric}!!!")
    return _metric_type


class DeepSearch(BaseANN):
    def __init__(self, metric, dim, method_param):
        ...
        self.name = 'deepsearch_(%s)' % (method_param)
        self.dir = 'deepsearch_indices'
        self.path = f'dim_{dim}_R_{self.R}_L_{self.L}.deepsearch'
    ...
    def fit(self, X):
        ...
        if self.path not in os.listdir(self.dir):
            p = deepsearch.Index("HNSW", dim=self.d,
                                 metric=self.metric, R=self.R, L=self.L)
            g = p.build(X)
            g.save(os.path.join(self.dir, self.path))
        g = deepsearch.Graph(os.path.join(self.dir, self.path))
        self.searcher = deepsearch.Searcher(g, X, self.metric, self.level)
